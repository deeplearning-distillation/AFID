...
parser.add_argument('--widen_factor', type=int, default=2, help='Model width.')
...
# Select the best network layer in advance
...
print("----------------------------" + str(d_rate))   
model = wideresnet_cifar.afid_wide_resnet(depth=args.depth, widen_factor=args.widen_factor,
...
# According to CIFAR
module = wideresnet_cifar.Fusion_module(128, num_classes, 8)
model.to(DEVICE)
module.to(DEVICE)
...
def eval(net,module, test_flag=False):
    ...
    print('Val_Loss: %.3f | val_Acc sub-1: %.3f%% | Val_Acc sub-2: %.3f%% | Val_Acc fused: %.3f%% |' % (train_loss / (b_idx + 1), 100. * correct_sub1 / total, 100. * correct_sub2 / total, 100. * correct_fused / total))
...
if __name__ == '__main__':
    ...
    if int(args.log_time) :
        folder_name = 'AFID_{}'.format(time_log)
    ...
    for epoch in range(RESUME_EPOCH, FINAL_EPOCH+1):
        ...
        f.write('EPOCH {epoch} \t'
                'Val_ACC_sub-1 : {acc_sub1:.4f} \t Val_ACC_sub-2 : {acc_sub2:.4f}\t' 
                'Val_ACC_fused : {acc_fused:.4f} \t \n'.format(
